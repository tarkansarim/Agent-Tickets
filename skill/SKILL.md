...
- `agent-ticket supervision status [--repo <path>|--ticket <id>|--claim <id>]` surfaces local active/stale supervision claims. `release`, `adopt`, and `steal` require a filter and only act on claims owned by the current supervisor (`--supervisor-id` or `AGENT_TICKETS_SUPERVISOR_ID`) or stale claims unless `--force` is supplied.
