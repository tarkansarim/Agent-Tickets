...
If the active repo you are working in is the same source repo named by the
ticket's `project:<name>` tag, you are already the owner lane for that ticket.
Move it to `Agent working`, fix from this source checkout, validate, comment
evidence, and close it in this chat. Do not run `dispatch`, `supervise`,
`supervise-batch`, or launch another same-repo owner worker unless the user
explicitly asks for delegation. Cross-repo routing is for tickets owned by a
different repo, for reporter/supervisor roles, or for an explicit user request
to use a separate worker.
...
When you hit an issue in **another** repo, file it (`agent-ticket new --project X …`) and optionally **dispatch** it — `agent-ticket dispatch <id>` (or `agent-ticket new … --dispatch`). Dispatch is **always explicit**; filing a ticket never auto-pings anyone. Do not use this route for a ticket whose `project:<name>` tag is the repo you are already owning in the current chat; fix that ticket in-place instead. What it does:

After `agent-ticket new` without `--dispatch`, the CLI prints an `Ask the user now` prompt when the ticket has a resolvable `project:<name>` tag in a routeable column. Ask that question immediately. If the user approves routing, use a guarded explicit route: `agent-ticket dispatch <id>` for best-effort existing-session contact, or `agent-ticket supervise <id> --full-permission` for the monitored route that may launch a deterministic owner worker. Do not run either command unless the user explicitly approves or originally requested dispatch.
...
agent-ticket supervision status --repo <repo>
...
## Notes
- **You may be told about open tickets automatically.** A notify hook surfaces `🎫 Open/New agent-ticket(s) for <repo>: …` for the repo you're working in. Claude Code uses `SessionStart` + `UserPromptSubmit`; Codex prints the baseline at `SessionStart` and uses a silent `Stop` cache refresh so Stop stdout stays empty and satisfies Codex's hook JSON contract. When you see ticket output, triage per the role rules above (fix-in-place if you own the repo, otherwise act on / hand off). You can always run `agent-ticket list --project <repo>` yourself too.
...
- `agent-ticket supervise <id>` is the deterministic owner-agent loop for higher-stakes cross-repo tickets. It resolves the owner repo, selects a bound contactable owner lane or a fresh deterministic Codex owner lane, sends or launches through guarded `agent-contact`/`agent-tmux`, can use visible full-permission Codex flags with `--full-permission`, polls ticket state, and runs closeout checks after closure. By default, single-ticket supervision probes the deterministic ticket-scoped session directly; unrelated same-repo panes are not treated as route identity for that ticket. Existing tmux panes are reused only when guarded `agent-contact` accepts a provider/session identity tied to the exact ticket session, explicit `--session`, or an active same-owner supervision claim; unbound contactable panes fail closed as stale/unsafe context. When `agent-contact` dry-run returns success such as `status=would_send`, `supervise` treats the pane as contactable; `pane_reason` is informational and pane-state safety stays owned by `agent-contact`. Codex fallback uses a new deterministic ticket-scoped session name from `--session-prefix`, the project tag, and the ticket id unless `--session` is supplied. Before requesting a Codex launch, it checks `agent-tmux codex-existing <repo> <session>` for that exact requested session; if the session already exists, it is reused only when guarded exact-session `agent-contact` accepts, otherwise dry-run and live mode fail closed with `existing session not contactable` instead of launching over the occupied name. If no exact owner pane exists, `--dry-run --json` reports a clear `first_contact` launch plan and separates known no-pane/no-current-target probe results under `absence_refusals`; those are absence evidence, not unsafe provider refusals. After a successful launch, it probes the exact launched session with guarded `agent-contact --dry-run` through the Codex startup window; visible trust or approval prompts block before route audit comments or ticket moves. It does not use `codex-latest` or `codex-resume-latest`, because latest-chat metadata is not deterministic owner-lane evidence and can point at stale review/subagent context; route output records `resume_latest: skipped` when it launches fresh. Before contacting or launching, it records a durable supervision claim keyed by canonical repo and ticket id. Active claims from another supervisor make dry-run and live mode fail closed with owner/origin/worker/heartbeat/expiry details. Use `--dry-run` first when checking route selection or claim state. Refusal/tooling states are reported and can file a local blocker unless `--no-tool-ticket` is set. Live `dispatch`, `supervise`, and `supervise-batch` contact/probe paths need `AGENT_CONTACT_TRUSTED_PROVIDER_ROOTS` and `AGENT_CONTACT_TRUSTED_LAUNCHER_ROOTS` in the environment.
- `agent-ticket supervise-batch` is the user-level board drain for multiple open tickets. It scans open non-`Done` tickets by default, skips `p3` and `Needs human` unless explicitly included, requires one exact `project:<repo>` tag per ticket, resolves repos with the same `repo_roots` rule, and groups by canonical resolved repo path so aliases for one source tree still route to one owner worker. For otherwise in-scope tickets, missing/multiple project tags and unresolved repos are blocking skipped tickets because they leave open work undrainable. Same-repo tickets go to that repo worker as one sequential queue with each item's exact project tag; different repos route in parallel. It checks durable supervision claims before route probing: active other-owner claims for the same canonical repo or overlapping tickets block dry-run/live mode with `already supervised` instead of a normal launch/contact plan unless `--force-supervision` is explicit. It uses only guarded `agent-contact` and `agent-tmux`: provider lanes are probed before choosing a route, exactly one safe contactable owner lane tied to an active same-owner provider/session claim identity is reused, known no-pane refusals count as absence, and busy/attached/pending/ambiguous/unknown provider refusals block the group. Contactable panes without matching provider/session claim identity block as unbound stale/unsafe context. If no safe bound lane and no unsafe provider state exists, one fresh deterministic Codex session is launched from `--session-prefix` and the repo basename only while holding a local per-repo batch lock through polling/closeout, and only after `agent-tmux codex-existing` returns a recognized no-existing-session result at route selection and again immediately before launch. After a successful launch, it probes the exact launched session with guarded `agent-contact --dry-run` through the Codex startup window; visible trust or approval prompts block before route audit comments or `New -> Triaging` moves, while ordinary busy/working states remain compatible with a worker processing the launch prompt. Latest-chat metadata is not queried for routing; launch route evidence records `resume_latest: skipped`, never an implicit resume target. Unknown inspection failures block. Unsafe existing same-repo sessions block the group instead of spawning a duplicate. Start with `--dry-run` to see grouping, skipped tickets, active/stale supervision claims, refusals, repo paths, provider/session choices, and launch/contact plans without ticket comments, moves, sends, or launches. After routing, it heartbeats the claim while polling and aggregates per-ticket closeout-check results; a ticket still open after the polling limit, moved to `Needs human`, or failed post-route `New -> Triaging` move makes the batch result fail.
- `agent-ticket supervision status [--repo <path>|--ticket <id>|--claim <id>]` surfaces local active/stale supervision claims. Default process-bound claim owners use `pid:<host>:<pid>`; when that local PID is dead, the claim is stale immediately and no TTL wait or `--force-supervision` is required. Explicit stable `--supervisor-id` claims remain TTL/recovery based. `release`, `adopt`, and `steal` require a filter and only act on claims owned by the current supervisor (`--supervisor-id` or `AGENT_TICKETS_SUPERVISOR_ID`) or stale claims unless `--force` is supplied.
