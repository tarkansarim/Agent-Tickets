...
present on this machine:
  ...
  * Codex      (~/.codex/hooks.json):       SessionStart -> "<hook> baseline",
                                            Stop -> "<hook> changes"   (Codex has no per-prompt event)
...
def register_codex():
    ...
    changed = _ensure(hooks, "Stop", "changes") or changed
    if changed:
        _save(path, data)
        print("  codex:  registered notify hook (SessionStart=baseline, Stop=changes) in %s" % path)
